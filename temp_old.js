...
class UnifiedTrackingSystem {
    ...
    async initializeTracking() {
        console.log('≡ƒÜÇ Unified Tracking System v2.1 initialized');
        ...
        if (!sessionStorage.getItem(pageViewKey)) {
            ...
            console.log('Γ£à First page_view event sent for this session');
        } else {
            console.log('ΓÅ¡∩╕Å Page view already sent for this session, skipping');
        }
        ...
        console.log('≡ƒôè Tracking Data:', this.trackingData);
    }
        ...
        return new Promise((resolve) => {
            ...
            const checkCFGeo = () => {
                if (window.CF_GEO && window.CF_GEO.country && window.CF_GEO.country !== 'Unknown') {
                    console.log('Γ£à CF_GEO loaded successfully:', window.CF_GEO);
                    resolve(window.CF_GEO);
                } else if (retries < maxRetries) {
                    retries++;
                    console.log(`ΓÅ│ Waiting for CF_GEO... (attempt ${retries}/${maxRetries})`);
                    setTimeout(checkCFGeo, delay);
                } else {
                    console.log('ΓÜá∩╕Å CF_GEO not available, using fallback data');
                    resolve({
                        country: 'Unknown',
                        city: 'Unknown', 
                        region: 'Unknown',
    ...
    retryLocationDetection() {
        ...
        const checkLocation = () => {
            ...
            if (window.CF_GEO && window.CF_GEO.country) {
                console.log('≡ƒîì CF_GEO loaded, updating location data');
            ...
            if (attempts < maxAttempts) {
                ...
                console.log('≡ƒîì CF_GEO not available after 5 seconds, using defaults');
            }
        };
    ...
    updateLocationData() {
        ...
        if (this.trackingData.location) {
            ...
            if (currentCountry === 'Unknown' || currentCountry === 'New Zealand' || 
                ...
                console.log('≡ƒîì Updating location data from CF_GEO (non-destructive)');
                ...
                console.log('≡ƒîì Location data updated (non-destructive):', this.trackingData.location);
            } else {
                console.log('≡ƒîì Location data already accurate, skipping update:', currentCountry, currentCity);
            }
        }
    }
    ...
    async initializeTrackingData() {
        ...
        console.log('≡ƒîì CF_GEO Data:', geo);
        console.log('≡ƒîì Geo available:', !!window.CF_GEO);
     ...
     */
    trackPageView() {
        console.log('≡ƒôè Tracking page view...');
    ...
    setupAPITracking() {
        ...
        window.addEventListener('beforeunload', () => {
            ...
            if (timeOnPage > 30 || scrollDepth > 25 || clicks > 3) {
                ...
                console.log('≡ƒôè Skipping page exit tracking - insufficient engagement');
            }
        });
    }
    ...
    updatePhoneNumbers() {
        ...
        console.log('≡ƒô₧ Phone numbers updated:', phoneData.display);
    }
    ...
    trackEvent(eventName, category, label, additionalData = {}) {
        ...
        console.log(`≡ƒôè Tracked: ${eventName} (${category}) - ${label}`);
    }
    ...
    shouldSendTrackingData(eventType, additionalData = {}) {
        ...
        if (!this.lastSentData || !this.lastSentData.timestamp) {
            console.log('≡ƒôè Sending first tracking event (no previous data):', eventType);
            return true;
        }

        // Always send significant events
        if (this.significantEvents.has(eventType)) {
            console.log('≡ƒôè Sending significant event:', eventType);
            return true;
        }
        ...
        if (hasLocationChange || hasEngagementChange || hasCustomerDataChange || hasServiceDataChange) {
            console.log('≡ƒôè Sending due to data changes:', {
        ...
        if (lastSent) {
            ...
            if (new Date(lastSent) < fiveMinutesAgo) {
                console.log('≡ƒôè Sending due to time threshold (5 minutes)');
                return true;
            }
        }
    ...
    async sendTrackingData(eventType, additionalData = {}) {
        ...
        if (!this.shouldSendTrackingData(eventType, additionalData)) {
            console.log('≡ƒôè Skipping tracking data send - no significant changes');
            return;
        }
        ...
        console.log('≡ƒôè SENDING TRACKING DATA:', {
            eventType: eventType,
        ...
        console.log('≡ƒÄ» Google Ads Debug:', {
        ...
        try {
            ...
            console.log('≡ƒôí Tracking API Response:', {
            ...
            if (!response.ok) {
                ...
                console.error('Γ¥î Tracking API Error Response:', errorText);
            } else {
                // Save current data as last sent data after successful send
                this.saveLastSentData(trackingPayload);
                console.log('≡ƒÆ╛ Saved tracking data for future comparison');
            }
        } catch (error) {
            console.error('Γ¥î Tracking API Error:', {
    ...
    async sendPaymentConfirmed(bookingData) {
        ...
        try {
            ...
            console.error('Γ¥î Payment confirmed API error:', error);
            return false;
        }
    }
    ...
    updateTrackingData(fieldName, value, eventType = 'data_update') {
        if (this.STANDARD_FIELDS[fieldName]) {
            ...
            console.log(`≡ƒôè Updated ${fieldName}:`, value);
